     ...
     m_encodeEnable(false)
{
    ...
    if (initParam.encodeParam.needEncode) 
    {
        ...
        {
            encodeCodec = avcodec_find_encoder(static_cast<AVCodecID>(initParam.encodeParam.codecId));
        }
        else
        {
        ...
        { 
            AV_LOG_E("Failed to alloc AVCodecContext with codec(%s)", initParam.encodeParam.codecName.c_str());
            return;
        }
        ...
        m_encodeCodecCtx->channels = av_get_channel_layout_nb_channels(m_encodeCodecCtx->channel_layout);
        ...
        if (int ret = avcodec_open2(m_encodeCodecCtx, encodeCodec, NULL); ret < 0)
        {
            ...
            AV_LOG_E("Failed to open codec (%s)\nerror:%s", initParam.encodeParam.codecName.c_str(), errors);
            return;
        }
        m_encodeEnable = true;
    ...
    if (initParam.decodeParam.needDecode)
    {
        ...
        {
            decodeCodec = avcodec_find_decoder(static_cast<AVCodecID>(initParam.decodeParam.codecId));
        }
        else
        {
